...
const searchCourses_auto = async searchText => {
  ...
  let matches = Object.values(course).filter(data => {
    ...
    return data.NICE_Role_Title.match(regex) || data.NICE_Role_Description.match(regex);
  });
...
const outputHtml_auto = matches => {
  if(matches.length > 0) {
    const html = matches.map(
      ...
      match =>   `
        <button class="card card-body mb-1" type="button" onclick="add_course(this.value, this.id)" value="${match.NICE_Role_Title}" id="${match.GraphDB1}"><p>${match.NICE_Role_Title}</p></button>
        `
